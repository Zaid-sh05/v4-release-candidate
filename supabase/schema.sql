-- Qanoni | قانوني V4 cloud schema
-- Run server-side in Supabase SQL Editor. All application tables remain private;
-- the backend and weekly updater use the service-role key only.
...
grant execute on function public.hybrid_search_legal_chunks(text,extensions.vector,text[],int) to service_role;
