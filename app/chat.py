...
def _apply_cognition_to_route(route:RouteResult, case, force_domain:str|None)->RouteResult:
    ...
    # Preserve specific router intents (deadline/fees/complaint/etc.). Cognition only
    # upgrades a generic legal_question when it understood the user's actual goal.
...
def _cognition_expansions(case)->list[str]:
    """Return bounded search expansions generated by the non-legal retrieval planner."""
    out=[]
...
def handle_chat(req:ChatRequest)->ChatResponse:
    ...
    # Smalltalk never consumes optional LLM cognition quota.
    ...
    # Cognition helps retrieval especially for long scenarios whose wording does not
    # exactly match statute vocabulary. It never supplies the legal answer itself.
