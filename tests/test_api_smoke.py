...
def main():
    with TestClient(app) as c:
        ...
        for msg,lang,expected in cases:
            r=c.post('/api/chat',json={'message':msg,'language':lang}); assert r.status_code==200,msg
            data=r.json(); assert data['route']['domains'][:len(expected)]==expected,(msg,data['route']['domains'])
